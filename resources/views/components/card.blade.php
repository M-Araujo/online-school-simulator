@props(['items', 'empty' => 'No records found.', 'row', 'authenticatedUser'])
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach ($items as $item)
                <div
                    class="course-card bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 flex flex-col justify-between">
                    <div>
                    ...
                    <div class="mt-auto">
                        <a href="{{ route('courses.show', $item->slug) }}" class="inline-block bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded hover:bg-blue-700 transition">
                            @if ($authenticatedUser->isStudent() && isUserEnrolledInCourse($authenticatedUser, $item))
                            Continue learning
                        @else
                           Details
                        @endif
